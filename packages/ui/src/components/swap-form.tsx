...
export default function SwapForm() {
  ...
  const ydOut = useMemo(() => {
    ...
    const fee = (ydGross * feeBpsValue) / 10_000n;
  ...
  const ethOut = useMemo(() => {
    ...
    if (parsedAmount <= 0n) return "0";
    const fee = (parsedAmount * feeBpsValue) / 10_000n;
  ...
  const feePercent =
    typeof feeBpsNumber === "number" ? (feeBpsNumber / 100).toFixed(2) : "--";
  ...
  return (
    <div className="card p-6">
      ...
      {rateNumber === null || feeBpsNumber === null ? (
        <p className="mt-2 text-xs text-text-muted">
          Waiting for onchain rate and fee from the Exchange contract...
        </p>
      ) : null}
